...
$TestA = Read-Host "`nPlease enter the software name."

try 
{
    # Replace occurrences of "TestA" in all files
    Write-Host "`nReplacing occurrences of 'TestA' with "$TestA" in all files.`n"
    ...
    Get-ChildItem -File -Recurse | ForEach-Object {
        ...
        try
        {
            (Get-Content $_.FullName) -replace 'TestA', $TestA | Set-Content $_.FullName
        } 
        catch 
        {
    ...
    Write-Host "`nRenaming files and directories using "$TestA":`n"
    ...
    # Get all files and directories containing "TestA" recursively
    Get-ChildItem -Recurse -Directory | ForEach-Object {
        $dirpath = $_.FullName
        $dirname = Split-Path  $dirpath -Leaf
        if ($dirname.Contains("TestA"))
        {
            $stack.Push($dirpath)
        ...
        foreach ($file in [Directory]::EnumerateFiles($dirpath)) 
        {
            ...
            if ($filename.Contains('TestA') -and -not $allPaths.Contains($file))
            {
                $stack.Push($file)
                $allPaths.Add($file)
            }
    ...
    while ($stack.Count) {
        ...
        if($filename.Contains('TestA') -and $pathExists)
        {
            $newName = $filename.Replace('TestA', $TestA)
